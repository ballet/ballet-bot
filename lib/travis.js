...
const mapPythonFeatureNameToDirectory = (
  featureInPython,
...
const getSuccessMetadataFromLog = (log) => {
  ...
  const omittedFeatureLogLine = logLines.find((line) =>
    line.includes(LINE_MARKERS.SUCCEEDED_WHILE_OMITTING_FEATURE)
  );

  if (omittedFeatureLogLine) {
    const omittedFeatureInPython = omittedFeatureLogLine
      .split(LINE_MARKERS.REDUNDANT_FEATURE_MARKER)[1]
      .trim();
    if (omittedFeatureInPython !== NONE_FEATURE) {
      omittedFeature = mapPythonFeatureNameToDirectory(omittedFeatureInPython, false);
    }
  }
